...
void Lines::add(GLfloat *vertex_data, GLfloat *color_data, size_t vertexCount,
                GLuint *indices_data, size_t indexCount) {
...
void Lines::load_to_gpu() {
  ...
  if (vertex_count) {
    ...
    glBufferData(GL_ARRAY_BUFFER, 3 * vertex_count * sizeof(GLfloat),
                 &points[0].v[0], GL_STATIC_DRAW);
  ...
  if (index_count) {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * index_count,
...
void Lines::set_shader_data(GLuint shader_programme,
                            const mat4 &referenceFrameMatrix) {
  glUniformMatrix4fv(model_matrix_location, 1, GL_FALSE,
                     referenceFrameMatrix.m);
}
...
void Shapes::addArrow(Lines &lines, const vec3 &from, const vec3 &to,
                      const vec3 &color) {
  vec3 one(1, 1, 1);

  // a = producto vectorial de one y vector me da vector a 45º de la
  // perpendicular

  // invertir eje, pasar a unitario, multiplicar por length, proyectar en a
  ...
  lines.add(&arrows[0].v[0], &colors[0].v[0], 2, &arrow_indices[0], 2);
}

void Shapes::addGrid(Lines &lines, const vec3 &from, const vec3 &to,
                     const vec3 &color, int divs) {
  int steps = divs + 1;
  ...
  lines.add(&vertices[0].v[0], &colors[0].v[0], vertices.size(), &indices[0],
            indices.size());
}
