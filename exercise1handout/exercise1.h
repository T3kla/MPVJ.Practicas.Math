...
struct Exercise1 {
  ...
  void init(int width, int height) {
    ...
    lines_shader_index =
        create_programme_from_files("lines_vs.glsl", "lines_fs.glsl");
    ...
    cubeLines.add(&cubeVertexPositions[0].v[0], &cubeVertexColors[0].v[0], 8,
                  &cubeLineIndices[0], 12 * 2);
    cubeLines.load_to_gpu();
    cubeMatrix = identity_mat4();
    cubePosition = vec3(0, 0, 0);

    Shapes::addArrow(referenceFrameLines, vec3(0, 0, 0), vec3(1, 0, 0),
                     vec3(1, 0, 0));
    Shapes::addArrow(referenceFrameLines, vec3(0, 0, 0), vec3(0, 1, 0),
                     vec3(0, 1, 0));
    Shapes::addArrow(referenceFrameLines, vec3(0, 0, 0), vec3(0, 0, 1),
                     vec3(0, 0, 1));
  ...
  void update() {
    ...
    referenceFrameLines.get_shader_locations(lines_shader_index);
    referenceFrameLines.set_shader_data(lines_shader_index,
  ...
  bool isLoopGo() { return !glfwWindowShouldClose(window); }
